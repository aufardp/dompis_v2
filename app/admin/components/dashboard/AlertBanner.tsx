...
function getAgeBadge(hours: number): { label: string; className: string } {
  ...
  return {
    label: `${Math.round(hours)}h`,
    className:
      'bg-red-500/15 text-red-600 dark:text-red-400 border border-red-500/20 animate-pulse font-semibold',
  };
}
      ...
      <div className='relative z-10 flex flex-col gap-4'>
        ...
        <div className='flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between'>
          ...
          <div className='flex items-center self-start sm:self-center'>
            <span className='rounded-full bg-red-600 px-3 py-1 text-xs font-semibold tracking-wide text-white shadow-sm shadow-red-500/30 dark:bg-red-500 dark:text-slate-950'>
              <span className='inline-flex items-center gap-1'>
                <AlertTriangle size={12} />
                {Math.round(maxOverdue)}H OVERDUE
              </span>
            </span>
          </div>
        </div>
        ...
        <div className='flex flex-wrap items-center gap-2.5 rounded-xl border border-red-200/60 bg-white/80 px-4 py-3 dark:border-red-500/10 dark:bg-red-950/40'>
          <span className='rounded bg-red-100/60 px-2 py-0.5 font-mono text-xs font-semibold tracking-wide text-red-700 dark:bg-red-500/10 dark:text-red-400'>
      ...
      <div className='relative z-10 flex flex-col gap-4'>
        ...
        <div className='flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between'>
          ...
          <div className='flex flex-wrap items-center gap-1.5 self-start lg:self-center'>
            <span className='rounded-md bg-cyan-600 px-2.5 py-1 text-[10px] font-semibold tracking-wide text-white dark:bg-cyan-500 dark:text-slate-950'>
              DIAMOND VIP
            </span>
            <span className='rounded-md border border-red-200 bg-red-100 px-2 py-1 text-[10px] font-semibold text-red-700 dark:border-red-500/20 dark:bg-red-500/10 dark:text-red-400'>
              OPEN: {statusSummary.open}
            </span>
            <span className='rounded-md border border-blue-200 bg-blue-100 px-2 py-1 text-[10px] font-semibold text-blue-700 dark:border-blue-500/20 dark:bg-blue-500/10 dark:text-blue-400'>
              ASSIGNED: {statusSummary.assigned}
            </span>
            <span className='rounded-md border border-orange-200 bg-orange-100 px-2 py-1 text-[10px] font-semibold text-orange-700 dark:border-orange-500/20 dark:bg-orange-500/10 dark:text-orange-400'>
        ...
        <div className='grid grid-cols-2 gap-2.5 md:grid-cols-4'>
          <div className='rounded-xl border border-cyan-100 bg-white p-3 shadow-2xs dark:border-cyan-500/10 dark:bg-cyan-950/40'>
            ...
            <p className='mt-1 text-xl font-semibold text-cyan-950 dark:text-cyan-100'>
              {tickets.length}
            </p>
          </div>
        ...
        <div className='space-y-2'>
          {visibleTickets.map((ticket, index) => {
            ...
            return (
              ...
              >
                {/* Index Count */}
                <span className='flex h-5 w-5 shrink-0 items-center justify-center rounded-md bg-cyan-100 text-[10px] font-semibold text-cyan-700 dark:bg-cyan-500/20 dark:text-cyan-400'>
                  {index + 1}
                </span>

                {/* Ticket ID */}
                <span className='rounded bg-slate-100 px-1.5 py-0.5 font-mono text-xs font-semibold text-slate-900 dark:bg-slate-800 dark:text-cyan-300'>
                  {ticket.ticketId}
                </span>
