...
export default function AdminManualTicketPage() {
  ...
  const handleSingleSubmit = async (e: React.FormEvent) => {
    ...
    if (data.success) {
      setMsg(
        `Berhasil: ${data.data.incident} ( ${data.data.id_ticket}) — jenis manual`,
      );
      setServiceNo('');
      setCustomerType('');
      setCustomerName('');
      ...
      setMsg(data.message || JSON.stringify(data.errors) || 'Gagal');
    }
    setLoading(false);
  };
  ...
  const parsePaste = () => {
    ...
    if (lines.length < 2) {
      setMassalErrors([
        'Minimal header + 1 baris data. Header: service_no,workzone,customer_type,customer_name,contact_phone,summary',
      ]);
      return;
    }
    ...
    if (missing.length) {
      setMassalErrors([
        `Header kurang: ${missing.join(', ')} (header saat ini: ${header.join(', ')})`,
      ]);
      return;
    }
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (cols.length < header.length) {
        errs.push(`Baris ${i + 1}: kolom kurang`);
        continue;
      }
      const row: ManualRow = {
      ...
      rows.push(row);
      if (rows.length >= 1000) {
        errs.push(`Dibatasi 1000 rows, sisa diabaikan`);
        break;
      }
    }
    setMassalRows(rows);
  ...
  const handleFile = async (file: File) => {
    ...
    if (name.endsWith('.csv')) {
      ...
      setPasteText(text);
      // auto parse after set
      setTimeout(() => {
        // trigger parse via pastetext state? manual
        const lines = text.split(/\r?\n/).filter((l) => l.trim());
        if (lines.length < 2) {
          setMassalErrors(['File kosong']);
          return;
        }
        const header = lines[0].split(',').map((h) => h.trim().toLowerCase());
        const idx: Record<string, number> = {};
        ...
        const missing = required.filter((c) => idx[c] === undefined);
        if (missing.length) {
          setMassalErrors([`Header kurang: ${missing.join(', ')}`]);
          return;
        }
        const rows: ManualRow[] = [];
        const errs: string[] = [];
      ...
      <div className='mx-auto max-w-5xl space-y-6 p-4'>
        ...
        {activeTab === 'single' ? (
          <form
            ...
            className='space-y-4 rounded-2xl border bg-white p-6 dark:border-slate-700 dark:bg-slate-800'
          >
            <div className='grid grid-cols-1 gap-4 md:grid-cols-2'>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder='DATIN / TSEL / VPN IP / INET / VOIP DLL'
                  className='mt-1 w-full rounded-lg border px-3 py-2 dark:border-slate-600 dark:bg-slate-900'
                />
              </div>
              <div>
            ...
            <div>
              ...
              <textarea
                ...
                rows={2}
                placeholder='Ringkasan gangguan, contoh: Gangguan DATIN LOSS'
                className='mt-1 w-full rounded-lg border px-3 py-2 dark:border-slate-600 dark:bg-slate-900'
              />
            </div>
            <div className='grid grid-cols-1 gap-4 md:grid-cols-2'>
          ...
          <div className='space-y-4 rounded-2xl border bg-white p-6 dark:border-slate-700 dark:bg-slate-800'>
            <div>
              ...
              <textarea
                ...
                rows={8}
                placeholder={`Format : service_no,workzone,customer_type,customer_name,contact_phone,summary

Contoh :
152312230908,RKT,INDIBIZ,PT ABC,0812,Gangguan Loss
12536253,DMO,ASTINET,PT DANU,0813,Pindah ONT`}
                className='mt-1 w-full rounded-lg border px-3 py-2 font-mono text-xs dark:border-slate-600 dark:bg-slate-900'
              />
              <div className='mt-2 flex gap-2'>
                <button
                  onClick={parsePaste}
                  className='rounded-lg bg-slate-800 px-4 py-2 text-sm text-white dark:bg-slate-700'
                >
                  Preview Paste
                </button>
                <label className='cursor-pointer rounded-lg border bg-white px-4 py-2 text-sm dark:bg-slate-900'>
                  Upload CSV/XLSX
                  <input
                    ref={fileRef}
                    type='file'
