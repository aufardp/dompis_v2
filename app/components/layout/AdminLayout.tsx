...
import { BucketCountContext, SetBucketCountContext } from '@/app/contexts/TicketSummaryContext';
...
export default function AdminLayout({
  ...
  const setBucketCount = useCallback((key: string, count: number) => {
    ...
    return (
      <div className='bg-bg flex min-h-screen items-center justify-center'>
        <div className='flex flex-col items-center gap-3'>
          ...
          <p className='text-sm text-(--text-secondary)'>Memuat sesi...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className='bg-bg flex min-h-screen overflow-x-clip'>
      <BucketCountContext.Provider value={bucketCounts}>
        <SetBucketCountContext.Provider value={setBucketCount}>
