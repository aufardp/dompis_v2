    ...
    <div ref={wrapperRef} className='relative flex items-center gap-1.5'>
      <button
        ...
        className={cn(
          ...
          open && activeTab === 'inbox' && 'border-sky-300 bg-sky-50 text-sky-900 dark:border-sky-500/30 dark:bg-sky-500/10 dark:text-sky-50',
        )}
      ...
      >
        ...
        <span
          className={cn(
            'inline-flex min-w-6 items-center justify-center rounded-full px-1.5 py-0.5 text-[10px] font-bold leading-none',
            inboxUnreadCount > 0
      ...
      <button
        ...
        className={cn(
          ...
          'border-slate-200 bg-white text-slate-700 shadow-sm hover:bg-slate-50 dark:border-white/10 dark:bg-white/5 dark:text-slate-200 dark:hover:bg-white/10',
          open && activeTab === 'diamond' && 'border-amber-300 bg-amber-50 text-amber-900 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-50',
        )}
        aria-label={`Open diamond alerts${diamondCriticalCount > 0 ? `, ${diamondCriticalCount} critical` : ''}`}
      >
