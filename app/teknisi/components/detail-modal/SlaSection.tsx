      ...
      <div className='rounded-2xl border border-slate-100 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-800'>
        <div className='mb-3 flex items-end justify-between'>
          <div>
            ...
            <p
              className={`text-2xl font-semibold ${ttrRemaining?.isOverdue ? 'text-red-600' : 'text-slate-800 dark:text-slate-100'}`}
            >
              {ttrRemaining?.label}
            </p>
          </div>
      ...
      {ttrRemaining && (
        <div className='flex items-center justify-between rounded-2xl border border-orange-200 bg-orange-50 px-4 py-3 dark:border-orange-500/20 dark:bg-orange-500/10'>
          ...
          <div className='text-right'>
            ...
            <p
              className={`text-xl font-semibold tabular-nums ${ttrRemaining.isOverdue ? 'text-red-600' : 'text-orange-700 dark:text-orange-300'}`}
            >
              {ttrRemaining.label}
            </p>
          </div>
