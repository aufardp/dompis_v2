...
const STATUS_ACTION_COLORS_DARK: Record<
  string,
  { bg: string; text: string }
> = {
  assigned: { bg: 'dark:bg-amber-500/15', text: 'dark:text-amber-400' },
  on_progress: { bg: 'dark:bg-blue-500/15', text: 'dark:text-blue-400' },
  pending: { bg: 'dark:bg-purple-500/15', text: 'dark:text-purple-400' },
  closed: { bg: 'dark:bg-green-500/15', text: 'dark:text-green-400' },
  ASSIGNED: { bg: 'dark:bg-amber-500/15', text: 'dark:text-amber-400' },
  ON_PROGRESS: { bg: 'dark:bg-blue-500/15', text: 'dark:text-blue-400' },
  PENDING: { bg: 'dark:bg-purple-500/15', text: 'dark:text-purple-400' },
  CLOSE: { bg: 'dark:bg-green-500/15', text: 'dark:text-green-400' },
};
...
export default function TicketCard({ ticket, onClick }: TicketCardProps) {
  ...
  return (
    <div
      ...
      className={`group cursor-pointer rounded-xl border border-l-4 border-slate-200 ${borderColor} bg-white p-3.5 shadow-sm transition-all hover:shadow-md sm:rounded-2xl sm:p-5 dark:border-slate-700 dark:bg-slate-800`}
    >
      <div className='flex flex-col gap-2 sm:gap-3'>
        {/* Incident Number */}
        <div className='flex items-center justify-between'>
          <span className='font-mono text-[13px] font-bold text-slate-500 dark:text-slate-400'>
            {ticket.ticket}
          </span>
          <svg
        ...
        <div className='flex flex-wrap items-center gap-1'>
            ...
            className={`shrink-0 rounded-full border px-2 py-0.5 text-[10px] font-semibold ${
              isClosed
                ...
                : status === 'ASSIGNED'
                  ...
                  : status === 'ON_PROGRESS'
                    ? 'border-blue-200 bg-blue-100 text-blue-700 dark:border-blue-500/20 dark:bg-blue-500/15 dark:text-blue-400'
                    : status === 'PENDING'
                      ? 'border-purple-200 bg-purple-100 text-purple-700 dark:border-purple-500/20 dark:bg-purple-500/15 dark:text-purple-400'
                      : 'border-slate-200 bg-slate-100 text-slate-600 dark:border-slate-700 dark:bg-slate-700/50 dark:text-slate-400'
            }`}
          >
            {isClosed
              ? 'Selesai'
          ...
          {ticket.jenisTiket && (
            <span className='shrink-0 rounded-full border border-slate-200 bg-slate-100 px-2 py-0.5 text-[10px] font-semibold text-slate-600 dark:border-slate-700 dark:bg-slate-700/50 dark:text-slate-400'>
              {ticket.jenisTiket}
            </span>
          )}
          <span
        ...
        {/* TITLE */}
        <h3 className='line-clamp-2 text-sm font-bold text-slate-800 dark:text-slate-100 sm:text-lg'>
          {title}
        </h3>
        ...
        <div className='flex flex-col'>
          ...
          <div className='flex items-start justify-between border-b border-slate-100 py-2 dark:border-slate-700/50'>
            <div className='min-w-0 flex-1 pr-3'>
              <p className='mb-0.5 text-[10px] font-bold tracking-wide text-slate-400 dark:text-slate-500 uppercase'>
                Pelanggan
              </p>
              <p className='truncate text-xs font-semibold text-slate-800 dark:text-slate-100 uppercase sm:text-sm'>
                {ticket.contactName || '-'}
              </p>
            </div>
            <div className='min-w-0 text-right'>
              ...
              {ticket.contactPhone ? (
                <a
                  ...
                  className='inline-flex items-center gap-1 truncate max-w-[140px] text-xs font-semibold text-emerald-600 hover:text-emerald-700 dark:text-emerald-400 dark:hover:text-emerald-300 sm:text-sm'
                >
                  <svg
          ...
          <div className='border-b border-slate-100 py-2 dark:border-slate-700/50'>
            <p className='mb-0.5 text-[10px] font-bold tracking-wide text-slate-400 dark:text-slate-500 uppercase'>
              Alamat
            </p>
            <p className='line-clamp-2 text-xs font-semibold text-slate-700 dark:text-slate-200 sm:text-sm'>
              {ticket.alamat || '-'}
            </p>
          </div>
          ...
          <div className='mt-1 flex items-center justify-between rounded-lg border border-orange-200 bg-orange-50 px-2 py-1.5 dark:border-orange-500/20 dark:bg-orange-500/10'>
            <span className='text-[10px] font-bold tracking-wide text-orange-500 dark:text-orange-400 uppercase'>
              ⚠ Max TTR
            </span>
            <span className='text-xs font-bold text-orange-700 dark:text-orange-300 sm:text-sm'>
        ...
        {/* ACTION FOOTER */}
        <div className='mt-1 flex items-center justify-between border-t border-slate-100 pt-2 dark:border-slate-700/50'>
          <div className='text-[10px] text-slate-400 dark:text-slate-500'>Klik untuk detail</div>

          {actionColor && (
            <span
              className={`rounded-lg px-2 py-1 text-[10px] font-semibold ${actionColor.bg} ${actionColor.text} ${STATUS_ACTION_COLORS_DARK[status]?.bg || ''} ${STATUS_ACTION_COLORS_DARK[status]?.text || ''}`}
            >
              {actionColor.icon}{' '}
