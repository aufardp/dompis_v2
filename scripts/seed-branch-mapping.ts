...
const DATA = [
  ...
  { sa: 'PGR', area: 'PUGERAN', branch: 'YOGYAKARTA', region: 'JATENG DIY' },
];

async function main() {
  console.log('🌱 Seeding region → branch → area → service_area...');
  ...
  for (const regionName of regions) {
    ...
    if (existing) {
      ...
      console.log(`  Region exists: ${regionName} (id=${existing.id_region})`);
    } else {
      const created = await prisma.region.create({ data: { nama_region: regionName } });
      regionMap[regionName] = created.id_region;
      console.log(`  Region created: ${regionName} (id=${created.id_region})`);
    }
  }

  // 2. Insert branches (under region)
  const branches = [...new Set(DATA.map(d => `${d.region}||${d.branch}`))];
  ...
  // 3. Insert areas (under branch)
  const areas = [...new Set(DATA.map(d => `${d.region}||${d.branch}||${d.area}`))];
  const areaMap: Record<string, number> = {};

  for (const areaKey of areas) {
    const [regionName, branchName, areaName] = areaKey.split('||');
    const branchKey = `${regionName}||${branchName}`;
    const branchId = branchMap[branchKey];

    const existing = await prisma.area.findFirst({
      where: { nama_area: areaName, branch_id: branchId },
    });
    if (existing) {
      areaMap[areaKey] = existing.id_area;
    } else {
      const created = await prisma.area.create({
        data: { nama_area: areaName, branch_id: branchId },
      });
      areaMap[areaKey] = created.id_area;
    }
  }

  // 4. Insert service_areas (under area)
  let createdCount = 0;
  let updatedCount = 0;

  for (const item of DATA) {
    const areaKey = `${item.region}||${item.branch}||${item.area}`;
    const areaId = areaMap[areaKey];

    const existing = await prisma.service_area.findFirst({
      where: { nama_sa: item.sa },
    });
    if (existing) {
      await prisma.service_area.update({
        where: { id_sa: existing.id_sa },
        data: { nama_sa: item.sa, area_id: areaId },
      });
      updatedCount++;
    } else {
  ...
  console.log(`\n✅ Done! Created ${createdCount} service areas, updated ${updatedCount} existing.`);
  console.log(`📊 Summary:`);
  console.log(`   Regions: ${Object.keys(regionMap).length}`);
  console.log(`   Branches: ${Object.keys(branchMap).length}`);
  console.log(`   Areas: ${Object.keys(areaMap).length}`);
  console.log(`   Service Areas: ${DATA.length}`);
}
